...
public class AccountService {
    ...
    public Account getAccountById(String accountId) {
        return accountRepository.findById(new AccountId(accountId))
                .orElseThrow(() -> new DomainException("ACCOUNT_NOT_FOUND", "Account not found: " + accountId));
    }
    ...
    public Account applyTrade(AccountId id, BigDecimal usdDelta, BigDecimal btcDelta) {
        return accountRepository.update(id, acc -> acc.withBalances(
                acc.usdBalance().add(usdDelta),
                acc.btcBalance().add(btcDelta)
        )).orElseThrow(() -> new DomainException("ACCOUNT_NOT_FOUND", "Account not found: " + id.value()));
    }
}
